                    ...
                    <li class="bg-white hover:bg-gray-50 transition-colors">
                        <div class="px-4 py-4 flex items-center sm:px-6">
                            <div class="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
                                <div class="truncate">
                                    <a href="{{route('tasks.show', ['task' => $item->id])}}" class="block flex">
                            ...
                            <div class="flex-grow flex flex-row gap-2 justify-end items-center">
                                @can('update', $item)
                                    <a href="{{route('tasks.update', ['task' => $item->id])}}">
                                ...
                                @endcan
                                @can('delete', $item)
                                    <form method='POST' action="{{route('tasks.destroy', ['task' => $item->id])}}" class="m-0">
                                        @csrf
                                        @method('DELETE')
